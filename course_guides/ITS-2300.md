# ITS 2300 Labs

Use the following guides to complete the question and answer section for the student's ITS 2300 class on Blackboard.

## Lab 1
- [Task 1 - Student Lab Tools](../tasks/Task-Student-Lab-Tools.md)
- [Task 2 - Connect to your GNS3 VM](../tasks/Task-Connect-to-GNS3-VM.md)
- [Task 3 - GNS3 Projects](../tasks/Task-GNS3-Projects.md)
- [Task 4 - Terminal Shell and File Management](../tasks/Task-Terminal-Shell-and-File-Management.md)
- [Task 5 - Shutdown project](../tasks/Task-Shutdown-GNS3.md)

## Lab 2
- [Task 1 - Gathering IP Information](../tasks/Task-Gathering-IP-Information.md)
- [Task 2 - Network Diagnostic Tools](../tasks/Task-Advanced-Network-Diagnostic-Commands.md)
- [Task 3 - Wireshark](../tasks/Task-Wireshark.md)
- [Task 4 - GitHub Account Creation](../tasks/Task-GitHub-Account-Creation.md)
- [Task 5 - Drawing Diagrams](../tasks/Task-Drawing-Diagrams.md)
